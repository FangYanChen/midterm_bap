...
class Piece:
    images = {
        ...
        'Kuzya': [pygame.image.load(r'chess_pieces/black_kuzya.jpg'), pygame.image.load(r'chess_pieces/white_kuzya.jpg')]
    }
...
class Kuzya(Piece):
    def __init__(self, board, color, square):
        super().__init__(board, color, square)
        self.img = self.images['Kuzya'][Piece.colors[color]] 
    ...
    def possible_moves(self, check):
        ...
        for d_row, d_col in squares:
            r, c = i + d_row, j + d_col
            middle_r, middle_c = i + d_row // 2, j + d_col // 2 
            if 0 <= r < self.board.rows and 0 <= c < self.board.columns:
                middle_square = self.board.get_square(middle_r, middle_c)
                target_square = self.board.get_square(r, c)
                if middle_square and middle_square.piece is not None and target_square:
                    self.add_if_legal(moves, target_square, check, pin)
