import pygame
from pieces import Queen, Rook, Pawn, King, Bishop, Knight, Kuzya
        ...
        if len(self.legal_moves) == 1:
            if len(self.opponent.legal_moves) == 1:
                ...
                for piece in self.opponent.legal_moves:
                    if isinstance(piece, Queen) or isinstance(piece, Rook) or isinstance(piece, Pawn) or isinstance(piece, Kuzya): #изменение
                        insufficient = False
            ...
            if insufficient:
                if len(self.opponent.legal_moves) == 2:
                    if not isinstance(remaining, Bishop) and isinstance(piece, Kuzya): #изменение
                        insufficient = False
                    else:
                        ...
                        if isinstance(opponent_remaining, Bishop) and isinstance(piece, Kuzya): #изменение
        ...
        if sq.column == self.promoting_pawn.square.column:
            if self.color == 'White':
                if sq.row == 8:
                    ...
                    promoted_piece = self.promoting_pawn.promote(Kuzya)
                else:
                    return 'Continue'
